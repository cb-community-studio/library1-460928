...
import borrow-historyFakerFactory from "./borrow-historyFakerFactory";
...
const Borrow-historyFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = borrow-historyFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("borrow-history").create(elem));
...
export default connect(null, mapDispatch)(Borrow-historyFakerDialogComponent);
