...
import seederData from "./borrow-history.seeder.json";
...
const Borrow-historySeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("borrow-history").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Borrow-historySeederDialogComponent);
